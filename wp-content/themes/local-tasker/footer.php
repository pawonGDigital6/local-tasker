					...
					<div class="top sm:pb-8 pb-6 border-b border-[#FFFFFF33] relative">
						<div class="logo flex justify-center">
							<img class="sm:w-[228px] w-[140px]"
								src="<?php echo site_url(); ?>/wp-content/uploads/2026/06/logo-white.png"
								alt="<?php echo esc_attr(get_bloginfo('name')); ?>">
						</div>
						<button type="button"
							class="absolute right-0 sm:top-2 top-0 max-sm:w-[17px] cursor-pointer quote-pop-closer"
							aria-label="<?php esc_attr_e('Close quote form', 'local-tasker'); ?>">
...
<div class="global-search-pop fixed inset-0 bg-lt-onyx z-99 pt-10 pb-[9rem] invisible opacity-0">
	<div class="container h-full w-full">
		<button type="button"
			class="global-search-pop__closer-holder w-[18px] h-[18px] ml-auto cursor-pointer search-pop-closer"
			aria-label="<?php esc_attr_e('Close search', 'local-tasker'); ?>">
					...
					<ul class="flex flex-wrap gap-4">
						<?php if (have_rows('social_links', 'options')): ?>
							<?php while (have_rows('social_links', 'options')):
								...
								// Label the icon-only link with its destination host, e.g. "www.facebook.com".
								$social_host = $link_url ? wp_parse_url($link_url, PHP_URL_HOST) : '';
								?>
								<li>
									<?php if ($link_url): ?>
										<a class="w-[32px] h-[32px] rounded-full bg-lt-text-secondary flex justify-center items-center  hover:translate-y-[-2px] p-1.5"
											...
											<?php echo $social_host ? 'aria-label="' . esc_attr($social_host) . '"' : ''; ?>>
											<?php
											if ($social_icon) {
