...
<div class="button-count-holder md:hidden block mb-11">
	<button type="button" id="lt-filter-open"
		...
		aria-expanded="false" aria-controls="lt-filters-sidebar">
		<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
			<g clip-path="url(#clip0_50513_1204)">
				<path d="M12.2477 2.33301H8.16504" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M5.83269 2.33301H1.75" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M12.2482 6.99902H6.99902" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M4.66621 6.99902H1.75" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M12.2482 11.665H9.33203" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M6.99917 11.665H1.75" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M8.16504 1.16602V3.49898" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M4.66602 5.83203V8.165" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
				<path d="M9.33203 10.498V12.831" stroke="#1E2939" stroke-width="1.16648" stroke-linecap="round"
					stroke-linejoin="round" />
			</g>
			<defs>
	...
	<span class="count-result text-caption-sm text-[#6A7282] block mt-4">
		Showing
		<span class="count"><?php echo esc_html($lt_result_range); ?></span> of <span
			class="total"><?php echo esc_html($lt_result_total); ?></span>
	</span>
</div>
<!-- Mobile: overlay -->
...
<!-- Sidebar -->
<aside id="lt-filters-sidebar" class="lt-shop-filters
		w-[240px] shrink-0
	...
	<form id="lt-filter-form" method="get" action="<?php echo esc_url(wc_get_page_permalink('shop')); ?>"
		class="p-0 md:p-0">
		<!-- Mobile: sidebar header -->
		<div class="md:hidden flex items-center justify-between px-5 py-4 border-b border-[#E9EAEC]">
			<span
				class="text-body font-bold text-lt-text-primary font-semi-ext"><?php esc_html_e('FILTERS', 'local-tasker'); ?></span>
			<button type="button" id="lt-filter-close" class="p-1 rounded hover:bg-lt-snow-drift transition-colors"
				aria-label="<?php esc_attr_e('Close filters', 'local-tasker'); ?>">
		...
		<div class="p-5 md:p-0 flex flex-col gap-4">
			...
			<!-- ── Availability ── -->
			<div class="lt-filter-group" data-filter-group="availability">
				<button type="button"
					class="lt-filter-group__toggle w-full flex items-center justify-between text-left gap-2 group"
					aria-expanded="true" aria-controls="filter-availability-body">
					<span class="font-semi-ext text-[13px] font-bold text-[#0a0d1a]">
						<?php esc_html_e('Availability', 'local-tasker'); ?>
					</span>
				...
				<ul id="filter-availability-body" class="mt-3 flex flex-col gap-2 list-none p-0 m-0">
					<!-- In Stock -->
					<li>
						<label class="flex items-center justify-between gap-2 cursor-pointer group/label">
							<span class="flex items-center gap-2">
								<input type="radio" name="filter_availability" value="in_stock"
									class="lt-filter-checkbox sr-only peer" <?php checked($active_availability, 'in_stock'); ?>>
								<span
								...
								<span
									...
									<?php esc_html_e('In Stock', 'local-tasker'); ?>
								</span>
							</span>
							<span class="text-[11px] text-[#6b7280] shrink-0" data-lt-avail-count="in_stock">
								<?php echo esc_html($count_in_stock); ?>
							</span>
						</label>
					</li>
					<!-- Out of Stock -->
					<li>
						<label class="flex items-center justify-between gap-2 cursor-pointer group/label">
							<span class="flex items-center gap-2">
								<input type="radio" name="filter_availability" value="out_of_stock"
									class="lt-filter-checkbox sr-only peer" <?php checked($active_availability, 'out_of_stock'); ?>>
								<span
									class="lt-filter-checkbox__ui w-4 h-4 rounded-full border border-[#D1D5DB] flex items-center justify-center shrink-0 peer-checked:bg-lt-brand peer-checked:border-lt-brand transition-colors duration-150"
									aria-hidden="true">
									<span class="hidden peer-checked:block w-1.5 h-1.5 rounded-full bg-lt-white"></span>
								...
								<span
									class="text-[13.5px] text-[#374151] peer-checked:text-lt-text-primary group-hover/label:text-lt-brand transition-colors duration-150">
									<?php esc_html_e('Out of Stock', 'local-tasker'); ?>
								</span>
							</span>
							<span class="text-[11px] text-[#6b7280] shrink-0" data-lt-avail-count="out_of_stock">
								<?php echo esc_html($count_out_of_stock); ?>
							</span>
						</label>
					</li>
				</ul>
			</div><!-- /.lt-filter-group availability -->
			...
			<div class="lt-filter-group" data-filter-group="category">
				<button type="button"
					class="lt-filter-group__toggle w-full flex items-center justify-between text-left gap-2 group"
					aria-expanded="true" aria-controls="filter-category-body">
					<span
						class="font-semi-ext text-[13px] font-bold text-[#0a0d1a]"><?php esc_html_e('Category', 'local-tasker'); ?></span>
					<svg class="w-4 h-4 text-lt-text-muted shrink-0 transition-transform duration-200 group-aria-[expanded=false]:rotate-180 mr-[-2px]"
						viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
						<path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
							stroke-linejoin="round" />
			...
			<div class="lt-filter-group" data-filter-group="price">
				<button type="button"
					class="lt-filter-group__toggle w-full flex items-center justify-between text-left gap-2 group"
					aria-expanded="true" aria-controls="filter-price-body">
					<span
						class="font-semi-ext text-[13px] font-bold text-[#0a0d1a]"><?php esc_html_e('Price Range', 'local-tasker'); ?></span>
					<svg class="w-4 h-4 text-lt-text-muted shrink-0 transition-transform duration-200 group-aria-[expanded=false]:rotate-180 mr-[-2px]"
						viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
						<path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
							stroke-linejoin="round" />
				...
				<div id="filter-price-body" class="mt-3">
					<div class="flex items-center gap-2">
						<div class="flex-1">
							<label for="lt-price-min"
								class="sr-only"><?php esc_html_e('Minimum price', 'local-tasker'); ?></label>
							<div class="relative">
								<span
									class="absolute left-3 top-1/2 -translate-y-1/2 text-caption-sm text-[#0A0D1A] pointer-events-none"
									aria-hidden="true">$</span>
								<input type="number" id="lt-price-min" name="min_price" min="0" step="1"
									value="<?php echo esc_attr($price_min); ?>" placeholder="0"
						...
						<div class="flex-1">
							<label for="lt-price-max"
								class="sr-only"><?php esc_html_e('Maximum price', 'local-tasker'); ?></label>
							<div class="relative">
								<span
									class="absolute left-3 top-1/2 -translate-y-1/2 text-caption-sm text-[#0A0D1A] pointer-events-none"
									aria-hidden="true">$</span>
								<input type="number" id="lt-price-max" name="max_price" min="0" step="1"
									value="<?php echo esc_attr($price_max); ?>" placeholder="300"
									class="placeholder:text-[#0A0D1A] w-full h-[34px] border border-[#e5e5df] rounded-md pl-6 pr-3 text-[13px] text-lt-text-primary bg-lt-white focus:outline-none focus:ring-2 focus:ring-lt-brand/30 focus:border-lt-brand">
							</div>
						</div>
					</div>
			...
			<div class="flex flex-col gap-3 pt-2">
				<a href="<?php echo esc_url($clear_url); ?>" id="lt-clear-filters"
