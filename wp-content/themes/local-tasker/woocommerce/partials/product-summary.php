...
?>
<div class="lt-product-summary" id="lt-product-summary" data-product-id="<?php echo esc_attr($product_id); ?>"
	...
	<!-- Tags -->
	<?php if ($tags): ?>
		<div class="lt-product-summary__tags flex flex-wrap gap-2 mb-4">
			<?php
			...
			foreach ($tag_ids as $tag_id):
				...
				?>
				<span
					class="inline-block text-caption-xs font-semibold uppercase tracking-[0.07em] text-lt-brand border border-[#0A65FC25] rounded-sm px-3 py-1 bg-[#0A65FC0D]">
	...
	<?php if ($rating_count > 0): ?>
		<div class="lt-product-summary__rating flex items-center gap-2 mb-4">
			<div class="flex items-center gap-[2px]"
				aria-label="<?php echo esc_attr(sprintf(__('Rated %s out of 5', 'local-tasker'), $average)); ?>">
				<?php for ($i = 1; $i <= 5; $i++):
					$fill = $i <= round((float) $average) ? '#F59E0B' : '#E5E7EB';
					?>
					<svg width="16" height="16" viewBox="0 0 16 16" fill="<?php echo esc_attr($fill); ?>"
						xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
						<path
							d="M8 1l1.854 4.146H14l-3.427 2.708 1.236 4.146L8 9.764l-3.809 2.236 1.236-4.146L2 5.146h4.146L8 1z" />
					</svg>
				<?php endfor; ?>
			</div>
			<a href="#lt-reviews"
				class="text-caption-sm text-lt-text-muted hover:text-lt-brand underline transition-colors duration-200">
				<?php
				printf(
	...
	<div class="lt-product-summary__pricing mb-1">
		<?php if ($has_price): ?>
			<div class="flex gap-1 items-end">
				<span class="price flex flex-col gap-y-1">
					<span id="lt-price-was"
						class="text-caption-md text-lt-text-muted line-through leading-none<?php echo ($is_on_sale && $regular_price_per_sqm_ex > 0) ? '' : ' hidden'; ?>"><?php echo wc_price($regular_price_per_sqm_ex); ?></span>
	...
	<!-- Trust line -->
	<div
		class="lt-product-summary__trust text-caption-sm text-lt-text-muted mb-5 m-0 flex flex-wrap gap-x-3 gap-y-1 items-center pt-5 mt-5 border-t border-[#E2DDD7]">
		<?php if ($sold_by_box): ?>
			<span class="metas flex gap-2 items-center">
				...
				<span class="text">
					<?php esc_html_e('Sold by the box', 'local-tasker'); ?>
				</span>
			</span>
		<?php endif; ?>
		<span class="metas flex gap-2 items-center">
		...
		<span class="metas flex gap-2 items-center">
			...
			<span class="text">
				<?php esc_html_e('Free delivery over $199', 'local-tasker'); ?>
			</span>
		</span>
	</div>
