...
<div class="lt-install-quote mb-5" id="lt-install-quote">
	<div class="rounded-2xl border border-[#ccc] overflow-hidden">
		<!-- Opt-in -->
		<label
			class="lt-install-quote__row flex items-start gap-4 px-6 py-4 cursor-pointer select-none transition-colors duration-200 hover:bg-lt-snow-drift">
			<input type="checkbox" id="lt-install-quote-input" name="lt_install_quote" value="1"
				class="lt-install-quote__input sr-only peer">
			<!-- Custom checkbox -->
			<span
				class="lt-install-quote__ui w-4 h-4 rounded border border-[#b3b3b3] flex items-center justify-center shrink-0 bg-lt-white peer-checked:bg-lt-brand peer-checked:border-lt-brand transition-colors duration-150 mt-1"
				aria-hidden="true">
			...
			<span class="leading-[1.5] text-[#0d0d0d]">
				<span
					class="font-semi-ext font-bold text-caption-md "><?php esc_html_e('I would also like an installation quote', 'local-tasker'); ?></span>
				<span class="block text-xs mt-1">A team member will get in touch regarding installation after checkout.</span>
			</span>
		</label>
		<!-- Need more info -->
