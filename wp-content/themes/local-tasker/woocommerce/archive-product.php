	...
	<div class="lt-shop-archive__bar border-b border-[#E9EAEC] bg-lt-white">
		<div class="container">
			<div class="flex flex-wrap items-center gap-x-4 gap-y-3 py-4">
				...
				<!-- Filter pills -->
				<nav class="lt-filter-pills flex flex-wrap items-center gap-2 flex-1" aria-label="<?php esc_attr_e( 'Quick filters', 'local-tasker' ); ?>">
					...
					?>
						<a
							...
							data-lt-pill="<?php echo esc_attr( $key ); ?>" class="lt-filter-pill text-caption-sm font-semibold px-4 py-2 rounded-full border transition-colors duration-200 <?php echo $is_active ? 'bg-lt-brand text-lt-white border-lt-brand' : 'bg-lt-white text-lt-text-secondary border-[#E9EAEC] hover:border-lt-brand hover:text-lt-brand'; ?>"
				...
				<div class="lt-shop-sort flex items-center gap-2 shrink-0 ml-auto">
					...
					<div class="relative">
						...
						<svg class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-lt-text-muted" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
		...
		<div class="lt-shop-archive__layout flex gap-8 items-start">
			...
			<div class="lt-shop-archive__main flex-1 min-w-0" id="lt-product-grid">
				...
				<?php if ( woocommerce_product_loop() ) : ?>
					...
					<div
						class="lt-shop-loadmore-wrap<?php echo $lt_cur_page >= $lt_max_pages ? ' is-hidden' : ''; ?>"
						data-lt-loadmore
						data-page="<?php echo esc_attr( $lt_cur_page ); ?>"
					...
					>
						<button type="button" class="lt-load-more" data-lt-loadmore-btn>
							<span class="lt-load-more__label"><?php esc_html_e( 'Load More Products', 'local-tasker' ); ?></span>
				...
				<?php else : ?>
					...
					<div class="lt-shop-archive__empty py-20 text-center">
						<p class="text-body-lg text-lt-text-muted"><?php esc_html_e( 'No products found.', 'local-tasker' ); ?></p>
