...
function local_tasker_scripts()
{
	...
	// style.css holds only the theme header comment - no rules - so it is
	// registered (some integrations reference the handle) but never enqueued.
	// Requesting it cost a full render-blocking round trip on every page.
	wp_register_style('local-tasker-style', get_stylesheet_uri(), array(), $version);
	wp_style_add_data('local-tasker-style', 'rtl', 'replace');
...
/**
 * Register the single shared Swiper copy.
 *
 * The files live in the theme's assets folder because node_modules is not
 * deployed - loading them from there 404s in production. Block bundles list
 * "swiper" as a dependency (see webpack.config.js), so WordPress enqueues this
 * copy automatically, in the right order, only on pages that actually use it.
 */
function lt_register_swiper()
{
	$swiper_version = '11.2.1';
...
add_action('wp_enqueue_scripts', 'lt_register_swiper', 1);

/**
 * Enqueue the Swiper stylesheet whenever something enqueued the Swiper script.
 *
 * Slider styles used to be bundled into every block's view.css, which shipped
 * the same ~18 KB of Swiper CSS once per slider block on the page.
 */
function lt_enqueue_swiper_style()
{
	if (wp_script_is('swiper', 'enqueued') || wp_script_is('swiper', 'to_do')) {
		wp_enqueue_style('swiper');
	}
}
add_action('wp_enqueue_scripts', 'lt_enqueue_swiper_style', 99);
add_action('wp_footer', 'lt_enqueue_swiper_style', 1);
