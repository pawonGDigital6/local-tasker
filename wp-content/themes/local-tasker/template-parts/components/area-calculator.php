<?php
/**
 * Area calculator — shared, context-free component.
 *
 * Renders the room-by-room area calculator card. Used by the WooCommerce single
 * product page (`woocommerce/partials/area-calculator.php`) and by the standalone
 * `acf-block/area-calculator` block, so the markup deliberately carries **no**
 * product / WooCommerce dependency.
 *
 * Every JS hook is a class scoped to the root element (`[data-lt-area-calc]`) —
 * there are no hard-coded child IDs — so any number of instances can live on the
 * same page without conflicting. Behaviour lives in
 * `src/global/js/components/area-calculator.js`.
 *
 * @param array $args {
 *     Optional. Component arguments.
 *
 *     @type string $uid           Root element id. Default a unique generated id.
 *     @type string $title         Card heading. Default "Area Calculator".
 *     @type string $subtitle      Small note on the right of the header. Default "Calculate your total area".
 *     @type float  $wastage       Wastage percentage added to the total. Default 10.
 *     @type bool   $show_cta      Whether to render the footer CTA. Default true.
 *     @type string $cta_label     CTA label. Default "Use This Area".
 *     @type string $cta_url       When set, the CTA renders as a link instead of a button.
 *     @type string $cta_target    CSS selector of an input the total gets written into (host-page integration).
 *     @type string $cta_scroll_to CSS selector scrolled into view after the CTA runs.
...
$lt_ac = wp_parse_args(
	...
	array(
		'uid' => wp_unique_id('lt-area-calc-'),
		'title' => __('Area Calculator', 'local-tasker'),
		'subtitle' => __('Calculate your total area', 'local-tasker'),
		'wastage' => 10,
		'show_cta' => true,
		'cta_label' => __('Use This Area', 'local-tasker'),
		'cta_url' => '',
		'cta_target' => '',
		'cta_scroll_to' => '',
		'class' => '',
...
// Wastage is authored as a percentage (10 = +10%); clamp to something sane.
$lt_ac_wastage = max(0, (float) $lt_ac['wastage']);
...
$lt_ac_cta_attrs = '';
if ($lt_ac['cta_target']) {
	$lt_ac_cta_attrs .= ' data-target="' . esc_attr($lt_ac['cta_target']) . '"';
}
if ($lt_ac['cta_scroll_to']) {
	$lt_ac_cta_attrs .= ' data-scroll-to="' . esc_attr($lt_ac['cta_scroll_to']) . '"';
}
?>
<div id="<?php echo esc_attr($lt_ac['uid']); ?>"
	class="lt-area-calc h-full bg-lt-white border border-[#E2DDD7] rounded-xl overflow-hidden<?php echo $lt_ac['class'] ? ' ' . esc_attr($lt_ac['class']) : ''; ?>"
	data-lt-area-calc data-wastage="<?php echo esc_attr($lt_ac_wastage); ?>"
	...
	<div class="flex items-center justify-between gap-3 px-5 py-4 bg-[#F6922207] border-b-[2px] border-[#F69222]">
		<div class="flex items-center gap-2 min-w-0">
			...
			<?php if ($lt_ac['title']): ?>
				<span
					class="text-base font-bold font-semi-ext text-lt-text-primary truncate"><?php echo esc_html($lt_ac['title']); ?></span>
			<?php endif; ?>
		</div>
		<?php if ($lt_ac['subtitle']): ?>
			<span
				class="text-caption-sm text-lt-text-muted shrink-0 max-xs:hidden"><?php echo esc_html($lt_ac['subtitle']); ?></span>
		<?php endif; ?>
	</div>
	<div class="sm:py-10 sm:px-6 p-4">
		...
		<template class="lt-area-calc__template">
		...
		<!-- Add row -->
		<button type="button"
			class="lt-area-calc__add mt-6 flex items-center gap-2 text-caption-sm font-semibold text-lt-brand hover:underline cursor-pointer">
		...
		<div class="mt-5 pt-4 border-t border-[#E2DDD7] grid grid-cols-2 gap-4">
			<div class="flex flex-col gap-1 min-w-0">
				...
				<span class="lt-area-calc__total text-2xl font-bold text-lt-text-primary leading-none break-words"
			...
			<div class="flex flex-col gap-1 min-w-0 text-right">
				<span class="text-caption-sm text-lt-text-muted">
					...
					printf(esc_html__('With %s%% wastage', 'local-tasker'), esc_html($lt_ac_wastage_label));
					?>
				</span>
				<span class="lt-area-calc__wastage text-body-lg font-medium text-lt-text-primary leading-none break-words"
					aria-live="polite">0 sqm</span>
				<span class="text-caption-sm text-lt-accent font-medium">
					<?php
					/* translators: %s: wastage percentage, e.g. "10". */
					printf(esc_html__('+%s%% included', 'local-tasker'), esc_html($lt_ac_wastage_label));
					?>
				</span>
			</div>
		</div>
		<?php if ($lt_ac['show_cta'] && $lt_ac['cta_label']): ?>
			<!-- Use This Area CTA -->
			<?php if ($lt_ac['cta_url']): ?>
				<a href="<?php echo esc_url($lt_ac['cta_url']); ?>"
					class="lt-area-calc__use block text-center w-full mt-5 rounded-md bg-lt-accent text-lt-white font-medium text-caption-md py-3 transition-colors duration-200 hover:bg-lt-accent/90 cursor-pointer"<?php echo $lt_ac_cta_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped above. ?>>
					<?php echo esc_html($lt_ac['cta_label']); ?>
				</a>
			<?php else: ?>
				<button type="button"
					class="lt-area-calc__use w-full mt-5 rounded-md bg-lt-accent text-lt-white font-medium text-caption-md py-3 transition-colors duration-200 hover:bg-lt-accent/90 cursor-pointer"
					aria-label="<?php esc_attr_e('Use this area in the calculator', 'local-tasker'); ?>"<?php echo $lt_ac_cta_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped above. ?>>
					<?php echo esc_html($lt_ac['cta_label']); ?>
				</button>
			<?php endif; ?>
		<?php endif; ?>
